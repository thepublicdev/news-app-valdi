...
export class SearchNews extends StatefulComponent<ViewModel, any> {
  state: State = {
    searchQuery: '',
    articles: [],
    isSearching: false,
  ...
  private handleSearchSubmit = async () => {
    if (this.state.searchQuery.trim().length > 0) {
      ...
      try {
        const articles = await this.viewModel.onSearch(this.state.searchQuery);
  ...
  onRender(): void {
    <view style={styles.container}>
      <view style={styles.header}>
        <view style={styles.backButton} onTap={this.viewModel.onBack}>
